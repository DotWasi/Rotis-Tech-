...
const testimonials = [
    {
        quote: "Usman's strategic vision and technical leadership were instrumental in getting the ROTIS prototype off the ground. His ability to manage complex technical challenges while keeping the team aligned is unparalleled.",
        name: "Jane Doe",
...
const TestimonialCard = ({ quote, name, title, company }: { quote: string, name: string, title: string, company: string }) => (
    <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700 flex flex-col">
         <svg className="w-10 h-10 text-brand-primary/50 mb-4" fill="currentColor" viewBox="0 0 32 32" aria-hidden="true">
                        ...
                        <div className="space-y-4">
                            {milestones.map((milestone, index) => (
                                <div key={index} className="flex items-center w-full">
                                    ...
                                    <div className="w-12 h-12 rounded-full bg-brand-primary border-4 border-gray-800 shrink-0 z-10 flex items-center justify-center font-bold">{index+1}</div>
                        ...
                        <div className="relative space-y-8 pl-12">
                            {milestones.map((milestone, index) => (
                                <div key={index} className="relative">
                                    <div className="absolute -left-10 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-brand-primary border-4 border-gray-800 shrink-0 z-10 flex items-center justify-center font-bold">{index+1}</div>
