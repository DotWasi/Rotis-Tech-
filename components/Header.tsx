...
const RotisLogo = () => (
    <div className="flex items-center space-x-3">
        <div className="w-12 h-12 flex-shrink-0">
             <svg viewBox="0 0 100 110" xmlns="http://www.w3.org/2000/svg" fill="#F85151">
                    ...
                    <div className="hidden md:block">
                        <div className="ml-10 flex items-baseline space-x-2">
                           ...
                           <div className="relative group">
                                ...
                                <div className="absolute right-0 top-full pt-2 w-48 z-20 opacity-0 group-hover:opacity-100 transform scale-95 group-hover:scale-100 transition-all duration-200 ease-in-out pointer-events-none group-hover:pointer-events-auto origin-top-right">
                                    <div className="bg-gray-800 rounded-md shadow-lg py-1">
                                     {navLinks.slice(7).map(link => (
                                         <NavLink key={link.name} to={link.path} className={({isActive}) => `block px-4 py-2 text-sm ${isActive ? 'text-brand-primary' : 'text-gray-300'} hover:bg-gray-700`}>
                                             {link.name}
                                         </NavLink>
                                     ))}
                                     </div>
